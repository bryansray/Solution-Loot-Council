SolutionLC = LibStub("AceAddon-3.0"):NewAddon("SolutionLC", "AceConsole-3.0", "AceEvent-3.0", "AceComm-3.0", "AceSerializer-3.0", "AceHook-3.0");
SolutionLC:SetDefaultModuleLibraries("AceEvent-3.0")
SolutionLC:SetDefaultModuleState(false)

SolutionLC_Mainframe = {}
SolutionLC_LootFrame = {}
SolutionLC_RankFrame = {}

local debug = false -- enable printing of debugging messages
local nnp = false
local superDebug = false -- extra debugging (very spammy)
local version = GetAddOnMetadata("SolutionLC", "Version")
...
local defaults = {
	...
	profile = {
		...
		autoAwardTo = "None",
		autoAwardReason = 1,
		-- below is the part of the db that's sent to others. Separate section to avoid sending unnecessary data.
		dbToSend = {
			selfVote = true,
			multiVote = true,
...
function SolutionLC:OnInitialize()
	self:RegisterChatCommand("slc", "ChatCommand")
    self:RegisterChatCommand("sollc", "ChatCommand")
	self:RegisterComm("SolutionLC")
	self.db = LibStub("AceDB-3.0"):New("SolutionLCDB", defaults, true)
	...
	LibStub("AceConfig-3.0"):RegisterOptionsTable("SolutionLC", self.options)
	
	-- add it to blizz options
	self.optionsFrame = LibStub("AceConfigDialog-3.0"):AddToBlizOptions("SolutionLC", "SolutionLC")	
	buttonsDB = db.dbToSend.buttons
end

function SolutionLC:OnEnable()
	self:debugS(" ")
	...
	local filterFunc = function(_, event, msg, player, ...)
		return strfind(msg, "[[SolutionLC]]:")
	end

	ChatFrame_AddMessageEventFilter("CHAT_MSG_WHISPER_INFORM", filterFunc)
end

function SolutionLC:OnDisable()
	self:UnregisterAllEvents()
	MainFrame:UnregisterAllEvents()
end

function SolutionLC:RefreshConfig()
	db = self.db.profile
end
...
-- Loads the MainFrame
------------------------------------------
function SolutionLC:MainFrame_OnLoad()
	MainFrame:Hide()
	------ Create content entries --------
	local entry = CreateFrame("Button", "$parentEntry1", ContentFrame, "SolutionLC_Entry"); -- Creates the first row
	entry:SetID(1); -- Set the ID
	entry:SetPoint("TOPLEFT", 4, -4) -- Set anchor
	for i = 2, MAX_DISPLAY do -- Create the rest of the rows
		local entry = CreateFrame("Button", "$parentEntry"..i, ContentFrame, "SolutionLC_Entry");
		entry:SetID(i);
	...
	for i = 1, MAX_ITEMS do
		local button = CreateFrame("Button", "SolutionLC_SessionButton"..i, MainFrame, "SolutionLC_SessionToggleButton");
		if i == 1 then
			button:SetPoint("TOPLEFT", MainFrame, "TOPRIGHT", 2, 0)
		elseif mod(i,10) == 1 then
			button:SetPoint("TOPLEFT", "SolutionLC_SessionButton"..i-10, "TOPRIGHT", 2, 0)		
		else
			button:SetPoint("TOP", "SolutionLC_SessionButton"..i-1, "BOTTOM", 0, -2)
		end
		button:SetID(i)
	end

	------ rightclick menu --------------
	menuFrame = CreateFrame("Frame", "SolutionLC_RightClickMenu", MainFrame, "UIDropDownMenuTemplate")
	UIDropDownMenu_Initialize(menuFrame, SolutionLC_Mainframe_RightClickMenu, "MENU")

	----------PopUp setups --------------
	-------------------------------------
	StaticPopupDialogs["SolutionLC_CONFIRM_ABORT"] = {
		text = "Are you sure you want to abort?",
		button1 = "Yes",
		button2 = "No",
		OnAccept = function()
			SolutionLC_Mainframe.abortLooting()
			self:SendCommMessage("SolutionLC", "stop", channel) -- tell the council to abort as well
	...
	StaticPopupDialogs["SolutionLC_CONFIRM_AWARD"] = {
		text = "Are you sure you want to give %s to %s?",
		button1 = "Yes",
		button2 = "No",
		OnAccept = function()
			SolutionLC_Mainframe.award()
		end,
		timeout = 0,
		enterClicksFirstButton = true,
		whileDead = true,
	...
	StaticPopupDialogs["SolutionLC_CONFIRM_USAGE"] = {
		text = "Do you want to use SolutionLC for this raid?",
		button1 = "Yes",
		button2 = "No",
		OnAccept = function()
			SolutionLC:debugS("CONFIRM_USAGE = true")
			isRunning = true;
			masterLooter = SolutionLC_Mainframe.getML()
			if db.autoAward and GetLootThreshold() > db.autoAwardQualityLower then
				SolutionLC:Print("Changing loot threshold to enable Auto Awarding.")
			...
			if #db.council < 1 then -- if there's no council
				...
				SolutionLC_RankFrame.show() -- show the rankframe
			end
		end,
		OnCancel = function ()
			SolutionLC:debugS("CONFIRM_USAGE = false")
			isRunning = false;
...
function SolutionLC.EventHandler(self2, event, ...)
		...
		if isMasterLooter and isRunning and #itemsToLootIndex == 0 and (IsInRaid() or nnp) then -- if we're masterlooter and addon is enabled or debug is on
			...
			if GetNumLootItems() >= 1 then -- if there's something to loot
				if not InCombatLockdown() then
					...
					for i = 1, GetNumLootItems() do -- run through the number of items in the loot window
						...
						if db.autoAward and LootSlotHasItem(i) and lootQuantity > 0 and (lootRarity >= db.autoAwardQualityLower and lootRarity <= db.autoAwardQualityUpper) then
							SolutionLC:AutoAward(i, db.autoAwardTo, GetLootSlotLink(i))
						...
						elseif db.autoLooting then
							if (LootSlotHasItem(i) or db.lootEverything) and lootQuantity > 0 and lootRarity >= GetLootThreshold() then -- Check wether we want to loot the item or not
								...
								if SolutionLC:LootBoE(GetLootSlotLink(i)) then
				...
				if #itemsToLootIndex > 0 then -- if there's anything in the table
					lootNum = 1;
					SolutionLC_initiateLoot(lootTable[lootNum]); -- initiate the loot
				end
			end
		end
	...
	elseif event == "GROUP_ROSTER_UPDATE" and (IsInRaid() or nnp) then
		masterLooter = SolutionLC_Mainframe.getML()
		isCouncil = SolutionLC_Mainframe.isCouncil()
		...
		if not lootFramesCreated and #lootTable > 0 then
			...
			if itemsLoaded then
				if not hasItemInfo then
					self:debugS("SolutionLC_LootFrame:Update() - "..event)
					hasItemInfo = true
					lootFramesCreated = true
					SolutionLC_LootFrame:Update(lootTable)
					if itemRunning then
						SolutionLC_Mainframe.prepareLootFrame(itemRunning);
					end
				end
			end
		end
...
-------------------------------------------
function SolutionLC:OnCommReceived(prefix, msg, distri, sender)
--[[List of commands:
	...
	if prefix == "SolutionLC" then
			...
			if not isMasterLooter or nnp then
				...
				if test then
					...
					if itemsLoaded then -- and only call .setupLoot if it is, otherwise wait for it to arrive (Event)
						self:debugS("SolutionLC_LootFrame:Update() - "..cmd)
						lootFramesCreated = true
						hasItemInfo = true
						SolutionLC_LootFrame:Update(lootTable)
					end
		...
		elseif cmd == 'council' then
			if not isMasterLooter then
				...
				self:debug("A non ML sent a councillist!")
			end
			...
			local test, c = self:Deserialize(object)
				if test then
					...
					if c["i"] == currentSession then -- only sort incoming rolls on our current page
						SolutionLC_Mainframe.sortTable(4, true) -- sort passes away, and let that function call the update
					end
				else
		...
		elseif cmd == 'remove' and (isCouncil or isMasterLooter) then
			if not isMasterLooter then
				...
				SolutionLC_Mainframe.removeEntry(session, name)
			else
		...
		elseif cmd == 'verTest' then
			...
			self:SendCommMessage("SolutionLC", "verTestReply "..self:Serialize({class, guildRank, version}) , "WHISPER", sender)
		...
		elseif cmd == 'verTestReply' then
			...
			if test then			
				...
				SolutionLC_VersionFrame:AddPlayer(c) -- and add the data to the versionFrame							
			else -- if old version
				SolutionLC_VersionFrame:AddPlayer({nil, sender, "Unknown", object})
			end
		...
		elseif cmd == "reRoll" then
			...
			local test, c = self:Deserialize(object)
			if test then 
				SolutionLC_LootFrame:Update(nil, {item = c[1], position = c[2]})
...
-------------------------------------------
function SolutionLC:debug(msg)
	if debug then 
		self:Print("debug: "..msg)
	end
	SolutionLC:DebugLogAdd(msg)
end

-------------- superDebug -----------------
-- Prints superDebug messages
-------------------------------------------
function SolutionLC:debugS(msg)
	if superDebug then
...
-------------------------------------------
function SolutionLC_Mainframe.showMainFrame()
	...
	if itemRunning then -- would hide the entries if a councilmember closes and trys to re open it
		...
		SolutionLC_Mainframe.Update(false)
	end
	MainFrame:Show()
end
...
-------------------------------------------
function SolutionLC_Mainframe.hideMainFrame()
...
--------------------------------------------
function SolutionLC_Mainframe.removeBtOnClick()
	if selection[1] then
		self:SendCommMessage("SolutionLC", "remove "..currentSession.." "..selection[1], channel)
		SolutionLC_Mainframe.removeEntry(currentSession, selection[1])
	else
...
--------------------------------------------
function SolutionLC_Mainframe.awardBtOnClick()
	StaticPopup_Show("SolutionLC_CONFIRM_AWARD", itemRunning, selection[1])
end

------------- abortLooting --------------------
-- Aborts the current looting session
-----------------------------------------------
function SolutionLC_Mainframe.abortLooting()
...
function SolutionLC_Mainframe.stopLooting()
...
-----------------------------------------------
function SolutionLC:ChatCommand(msg)
	...
	elseif input == 'config' then
		...
		LibStub("AceConfigDialog-3.0"):Open("SolutionLC")
	...
	elseif input == 'show' then
		if not IsInRaid() or isCouncil or isMasterLooter or nnp then -- only the right people may see the window during a raid since they otherwise could watch the entire voting
			SolutionLC_Mainframe.showMainFrame()
		else
	...
	elseif input == 'test' then
		if (IsInRaid() or nnp) and arg then
			SolutionLC_Mainframe.getML()
			SolutionLC_Mainframe.raidTestFrames(tonumber(arg))
		else
			if arg then 
				self:Print("You can only raid test when in a raid and are the group leader/assistant.")
				self:Print("Use \"/slc test\" for solo test.")
				return;
			end
			SolutionLC_Mainframe.testFrames()
		end

	elseif input == "add" and nnp then
		local _, totalIlvl = GetAverageItemLevel()
		local gear1, gear2 = SolutionLC.getCurrentGear(lootTable[currentSession]);
		...
		SolutionLC:OnCommReceived("SolutionLC", "add "..self:Serialize(toAdd), channel, "Someone else")
	 
	elseif input == 'version' or input == "v" or input == "ver" then
		self:EnableModule("SolutionLC_VersionFrame")
	elseif input == "history" or input == "his" then
		self:EnableModule("LootHistory")
...
--------------------------------------
function SolutionLC_initiateLoot(item)
	...
	if isRunning or nnp then -- only do stuff if we're using the addon or debugging
		...
		if itemRunning then -- if we're already trying to handle an item
			...
			self:SendCommMessage("SolutionLC", "council "..self:Serialize(currentCouncil), channel) -- let the members know if they're councilmen
			mlDB = db.dbToSend -- create the master looter db
			self:SendCommMessage("SolutionLC", "mlDB "..self:Serialize(mlDB), channel) -- and send 
			self:SendCommMessage("SolutionLC", "lootTable "..self:Serialize(lootTable), channel) -- tell everyone to do the same
			self:SendCommMessage("SolutionLC", "start "..lootNum, channel) -- tell the council we've started
			self:debug("Added item: "..item)			
			BtClose:SetText("Cancel Looting")
			SolutionLC:announceConsideration() -- announce it if it's on
			self:debugS("SolutionLC_LootFrame:Update() - SolutionLC_initiateLoot()")
...
--------------------------------------
function SolutionLC:initiateNext(item)
	itemRunning = item
	SolutionLC:announceConsideration() -- announce it if it's on
	self:SendCommMessage("SolutionLC", "start "..lootNum, channel) -- tell the council to start on the next item
...
--------------------------------------
function SolutionLC_Mainframe.prepareLootFrame(item)
	...
	if isMasterLooter or isCouncil or isTesting then
		...
		SolutionLC_Mainframe:UpdateSessionButtons()
		SolutionLC_Mainframe.showMainFrame()
	else
...
-----------------------------------------------
function SolutionLC_Mainframe.ShowCurrentItemTooltip()
	if itemRunning then
...
-----------------------------------------------
function SolutionLC_Mainframe.ShowCurrentGearTooltip(id)
	GameTooltip:SetOwner(MainFrame, "ANCHOR_CURSOR")
	if id then
...
-----------------------------------------------
function SolutionLC_Mainframe.ShowCurrentGear2Tooltip(id)
	if id then
		id = id + offset -- handle the offset
...
-----------------------------------------------------------------
function SolutionLC_Mainframe.toolMouseLeave()
	GameTooltip:Hide()
end
...
-------------------------------------------
function SolutionLC:GetGuildRank()
...
-------------------------------------------
function SolutionLC_Mainframe.setClassIcon(frame, class)
	if class then
...
-------------------------------------------
function SolutionLC_Mainframe.setCharName(frame, class, text)
...
-------------------------------------------
function SolutionLC_Mainframe.setCurrentGear(item1, item2, gear1Frame, gear2Frame)
	if item1 then
...
---------------------------------------
function SolutionLC.getPlayerRole(arg)
	local role;
	if not arg then
...
--------------------------------------------
function SolutionLC.handleResponse(response, frame)
-- entryTable order: (playerName, rank, role, totalIlvl, response, gear1, gear2, votes, class, color[], haveVoted, voters[], note)
	...
	SolutionLC:debugS("responseID = "..id)
	local _, totalIlvl = GetAverageItemLevel()
	local gear1, gear2 = SolutionLC.getCurrentGear(lootTable[id]);
	local _, class = UnitClass("player"); -- non-localized class name
	...
	SolutionLC_Mainframe.Update(true);
	self:SendCommMessage("SolutionLC", "add "..self:Serialize(toAdd), channel) -- tell everyone else to add it
end
...
-----------------------------------------------------------------
function SolutionLC_Mainframe.isSelected(id)
	id = id + offset -- handle the offset
...
---------------------------------------------
function SolutionLC_Mainframe.updateSelection(id, update)
...
-------------------------------------------
function SolutionLC_Mainframe.removeEntry(session, name)
	...
	if id then
		...
		SolutionLC_Mainframe.Update(true); -- update
		if selection and selection[1] == name then
			SolutionLC_Mainframe.updateSelection(0, true); -- and clear the selection if we had selected the one to be removed
		end
	else 
...
-------------------------------------------
function SolutionLC_Mainframe.vote(id)
...
-----------------------------------------
function SolutionLC_Mainframe.voteOther(session, name, devote, voter)
...
-----------------------------------------
function SolutionLC_Mainframe.award(reason)
	...
	elseif isTesting then
		if IsInRaid() or nnp then -- continue
			...
			elseif lootNum < #itemsToLootIndex then -- if there's more items to loot
				...
				lootNum = lootNum + 1; 
				isTesting = true
				SolutionLC:initiateNext(lootTable[lootNum]); -- start it
			else -- if there's not
				self:Print("Raid Test concluded.")
				self:SendCommMessage("SolutionLC", "stop", channel) -- tell the council to stop as well
				SolutionLC_Mainframe.abortLooting()
				SolutionLC_Mainframe.stopLooting()
				CloseButton_OnClick(); 
			end
		...
		for i = 1, GetNumGroupMembers() do
			if GetMasterLootCandidate(itemsToLootIndex[lootNum], i) == selection[1] then
				...
				if db.sendHistory and table then
					self:SendCommMessage("SolutionLC", "award "..selection[1].." "..self:Serialize(table), channel)
				end
...
--------------------------------------------------
function SolutionLC_Mainframe.getGuildRankNum(playerName)
	GuildRoster();
	if playerName then
...
--------------------------------------------------
function SolutionLC_Mainframe.getLowestItemLevel(entry)
	if entry then
		if entry[7] then
...
--------------------------------------------------
function SolutionLC_Mainframe.sortTable(id, specialSort)
...
-- entryTable order: (playerName, rank, role, totalIlvl, response, gear1, gear2, votes, class, color[], haveVoted, voters[])
	...
	elseif (id == 1) then -- Guild Rank sorting (numerically)
		table.sort(entryTable[currentSession], function(v1, v2)
			...
			if sortMethod == "desc" then
				return (v1 and SolutionLC_Mainframe.getGuildRankNum(v1[1]) > SolutionLC_Mainframe.getGuildRankNum(v2[1]))
			else
				return (v1 and SolutionLC_Mainframe.getGuildRankNum(v1[1]) < SolutionLC_Mainframe.getGuildRankNum(v2[1]))
			end
		end)
	...
	elseif (id == 5) then -- Item Level Sorting (lowest item level at the top - largest upgrade so to speak)
		table.sort(entryTable[currentSession], function(v1, v2)
			...
			if sortMethod == "desc" then
				return ((v1 ~= nil) and (v2 == nil or ((SolutionLC_Mainframe.getLowestItemLevel(v1) ~= -1) and (SolutionLC_Mainframe.getLowestItemLevel(v1) > SolutionLC_Mainframe.getLowestItemLevel(v2)))))
			else
				return ((v1 ~= nil) and (v2 == nil or ((SolutionLC_Mainframe.getLowestItemLevel(v1) ~= -1) and (SolutionLC_Mainframe.getLowestItemLevel(v1) < SolutionLC_Mainframe.getLowestItemLevel(v2)))))
			end
		end)
...
-------------------------------------------------------
function SolutionLC_Mainframe.getML()
...
-------------------------------------------------------
function SolutionLC_Mainframe.isCouncil()
...
----------------------------------------
function SolutionLC_Mainframe.Update(update)
	...
	for i = 1, MAX_DISPLAY do 
		...
		if update and entryTable[currentSession][line] then -- if there's something at a given entry and we want to update
			...
			SolutionLC_Mainframe.setCharName(getglobal("ContentFrameEntry"..i.."CharName"),entry[9], entry[1]) -- set the charName and color
			...
			SolutionLC_Mainframe.setClassIcon(getglobal("ContentFrameEntry"..i.."ClassTexture"), entry[9]); -- set the class-texture
			...
			SolutionLC_Mainframe.setCurrentGear(entry[6], entry[7], getglobal("ContentFrameEntry"..i.."CurrentGear1HoverCurrentGear1"),getglobal("ContentFrameEntry"..i.."CurrentGear2HoverCurrentGear2"));
...
-------------------------------------
function SolutionLC_Mainframe.testFrames()
	...
	for j = 1, 9 do
		...
		if itemLink ~= nil then				
			...
			isTesting = true
			mlDB = db.dbToSend
			SolutionLC:announceConsideration()
			SolutionLC_LootFrame:Update(lootTable)	
			SolutionLC_Mainframe.prepareLootFrame(itemLink)
			lootFramesCreated = true
...
-- Tests the frames
-------------------------------------
function SolutionLC_Mainframe.raidTestFrames(arg)
	...
	if IsInRaid() or nnp then
		if UnitIsGroupLeader("player") or UnitIsGroupAssistant("player") or nnp then -- if we're the group leader/assistant
			...
			lootNum = 1;
			hasItemInfo = true
			SolutionLC_initiateLoot(lootTable[lootNum])
...
-----------------------------------
function SolutionLC:buttonsToDefault()
...
-- Sets the button/response options back to default
-----------------------------------
function SolutionLC:announceToDefault()
...
function SolutionLC:otherAwardReasonsToDefault()
...
----------------------------------
function SolutionLC:GetVersion()
	return version;
end
...
-- Displays the voters
----------------------------------
function SolutionLC_Mainframe:voteHover(id)
...
---------------------------------------------
function SolutionLC_Mainframe:PeopleToRollHover()
	SolutionLC:debugS("Mainframe.PeopleToRollHover()")
	local peopleToRoll = {}	
	for i = 1, GetNumGroupMembers() do
...
-----------------------------------------
function SolutionLC:LootOnClick(button)
	...
	elseif db.altClickLooting and IsAltKeyDown() and not IsShiftKeyDown() and not IsControlKeyDown() then
		...
		if #lootTable == 1 then -- only initiate the first item
			lootNum = 1
			SolutionLC_initiateLoot(GetLootSlotLink(button.slot))
...
-- to the entryFrame.
----------------------------------------------
function SolutionLC:GetItemsFromMessage(msg, sender)
	SolutionLC:debugS("GetItemsFromMessage("..tostring(msg)..", "..tostring(sender)..")")
		...
		if theItem  then -- If they entered a valid item
			...
			for i = 1, GetNumGroupMembers() do
				...
				if name == sender then
					...
					role = SolutionLC.getPlayerRole(role)
			...
			SolutionLC_Mainframe.removeEntry(lootNum, sender) -- just remove it right away to avoid doubles (.removeEntry will handle errors)
			self:SendCommMessage("SolutionLC", "remove "..lootNum.." "..sender, channel)
			tinsert(entryTable[lootNum], toAdd[1])
			self:SendCommMessage("SolutionLC", "add "..self:Serialize(toAdd), channel)	
			SolutionLC_Mainframe.Update(true)
			self:Print("Item received and added from "..sender..".")
			-- tell the player what they've been added as
			SendChatMessage("[SolutionLC]: Acknowledged as \""..buttonsDB[responseNum]["response"].."\"", "WHISPER", nil, sender)
		end
	end
end
...
--------------------------------------
function SolutionLC_Mainframe:NameHover(id)
	...
	if lootDB[name] then -- if they're even in the db
		for i = #lootDB[name], 1, -1 do -- start with the end, and count downwards
			if lootDB[name][i]["responseID"] == 1 then -- the last mainspec loot awarded
				local dateString = SolutionLC:GetNumberOfDaysFromNow(lootDB[name][i]["date"])
				local firstDateString = SolutionLC:GetNumberOfDaysFromNow(lootDB[name][1]["date"])
...
---------------------------------------------------------------
function SolutionLC:GetNumberOfDaysFromNow(date)
...
----------------------------------
function SolutionLC:GetVariable(arg)
...
----------------------------------------------------
function SolutionLC:LootBoE(item)
...
function SolutionLC:announceConsideration()
...
----------- FilterPasses ---------------
function SolutionLC_Mainframe.FilterPasses(get)
	if get then
...
------------- SendWhisperHelp ---------------
function SolutionLC:SendWhisperHelp(playerName)
	SolutionLC:debugS("SendWhisperHelp("..tostring(playerName)..")")
	local msgToSend, msgToSend1 = "", "[SolutionLC]: To be added on the consideration list simply link your item(s) that would be replaced by the item under consideration followed by a keyword from the following list depending on your desire:";
	SendChatMessage(msgToSend1, "WHISPER", nil, playerName)
	for i = 1, db.dbToSend.numButtons do
		msgToSend = "[SolutionLC]: "..buttonsDB[i]["text"]..":      " -- i.e. MainSpec/Need:
...
-------------- DisplayNote -------------------
function SolutionLC_Mainframe:DisplayNote(id)
	GameTooltip:SetOwner(MainFrame, "ANCHOR_CURSOR")
...
-------------- DebugLogAdd --------------------
function SolutionLC:DebugLogAdd(msg)
...
-------------- ChangeSession ------------------
function SolutionLC_Mainframe:ChangeSession(id)
	SolutionLC:debugS("Mainframe:ChangeSession("..tostring(id)..")")
	currentSession = id
	SolutionLC_Mainframe.updateSelection(0, true);
	SolutionLC_Mainframe.prepareLootFrame(lootTable[id])
end

-------------- SessionButtonOnEnter -----------
function SolutionLC_Mainframe:SessionButtonOnEnter(id)
...
function SolutionLC_Mainframe:UpdateSessionButtons()
	for i = 1, MAX_ITEMS do
		local button = getglobal("SolutionLC_SessionButton"..i)
		if lootTable[i] then
			local _,_,_,_,_,_,_,_,_, texture = GetItemInfo(lootTable[i]);
			if not texture then
				texture = "Interface\InventoryItems\WoWUnknownItem01"
			end
			getglobal("SolutionLC_SessionButton"..i.."NormalTexture"):SetVertexColor(1,1,1)
			if i == lootNum then
			...
			elseif i < lootNum then
				...
				getglobal("SolutionLC_SessionButton"..i.."NormalTexture"):SetVertexColor(0.3, 0.3, 0.3)
			else
				getglobal("SolutionLC_SessionButton"..i.."NormalTexture"):SetVertexColor(0.3, 0.3, 0.3)
				button:SetBackdropBorderColor(0,0,0,1) -- white
			end
...
------------ EntryOnClick ---------------------
function SolutionLC_Mainframe:EntryOnclick(frame, button)
	if not SolutionLC_Mainframe.isSelected(frame:GetID()) then
		SolutionLC_Mainframe.updateSelection(frame:GetID())
	end
...
----------- RightClickMenu -------------------
function SolutionLC_Mainframe_RightClickMenu(menu, level)
	if level == 1 then
		...
		if currentSession == lootNum then -- greyout award buttons if we can't award.
			UIDropDownMenu_AddButton({text = "Award", notCheckable = true, func = function() if currentSession == lootNum then StaticPopup_Show("SolutionLC_CONFIRM_AWARD", itemRunning, selection[1]); else self:Print("You cannot award this item yet.")end; end, }, level);
		...
		UIDropDownMenu_AddButton({text = "Remove from consideration", notCheckable = true, func = function() self:SendCommMessage("SolutionLC", "remove "..currentSession.." "..selection[1], channel); SolutionLC_Mainframe.removeEntry(currentSession, selection[1]); end, }, level);
	
	elseif level == 2 then
		local value = UIDROPDOWNMENU_MENU_VALUE
		if value == "AWARD_FOR" then
			for k,v in pairs(db.otherAwardReasons) do
				UIDropDownMenu_AddButton({text = v.text, notCheckable = true, func = function() SolutionLC_Mainframe.award(v)end, }, level)
			end
					...
					func = function()
						for x,y in pairs(entryTable[currentSession]) do
							if y[1] == selection[1] then 
								...
								SolutionLC_Mainframe.Update(true);
								self:SendCommMessage("SolutionLC", "change "..self:Serialize({currentSession,x,k}), channel)
								return;
							end
						end
					end,}, level)	
		...
		elseif value == "REANNOUNCE" then
			...
			UIDropDownMenu_AddButton({text = "This item", notCheckable = true,
				func = function()
					self:SendCommMessage("SolutionLC", "reRoll "..self:Serialize({lootTable[currentSession], currentSession}), "WHISPER", selection[1])
					self:SendCommMessage("SolutionLC", "remove "..currentSession.." "..selection[1], channel)
					SolutionLC_Mainframe.removeEntry(currentSession, selection[1])
				end,
			}, level);
			...
			UIDropDownMenu_AddButton({text = "All items", notCheckable = true,
				func = function()
					...
					self:SendCommMessage("SolutionLC", "lootTable "..self:Serialize(lootTable), "WHISPER", name)
					for i = 1, #entryTable do
						self:SendCommMessage("SolutionLC", "remove "..i.." "..name, channel)
						SolutionLC_Mainframe.removeEntry(i, name)
					end
				end,
			}, level);
			UIDropDownMenu_AddButton({text = "", notCheckable = true, disabled = true}, level);
	...
	elseif level == 3 then
		if GetNumGroupMembers() > 0 then
			...
			UIDropDownMenu_AddButton({text = GetUnitName("player",true), notCheckable = true,
				func = function()
					self:SendCommMessage("SolutionLC", "lootTable "..self:Serialize(lootTable), "WHISPER", GetUnitName("player",true))
					for i = 1, #entryTable do
						SolutionLC_Mainframe.removeEntry(i, GetUnitName("player",true))
					end
				end,}, level);
		end
	end
...
-------------- AutoAward --------------------
function SolutionLC:AutoAward(index, awardTo, itemLink)
	...
	if UnitName("player") == awardTo then -- just take it
		...
		if db.sendHistory and table then
			...
			self:SendCommMessage("SolutionLC", "award "..awardTo.." "..self:Serialize(table), channel)
		end
